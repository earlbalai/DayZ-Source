class CfgVehicles {
	...
	class zZombie_Base : Citizen1 {
		...
		class Eventhandlers {
			...
			local = "if(_this select 1) then { if(isDedicated) then {[_this select 0] call zombie_findOwner;} else {[(position (_this select 0)),(_this select 0),true] execFSM '\z\AddOns\dayz_code\system\zombie_agent.fsm'}};";
		};
