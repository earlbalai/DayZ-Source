...
if ((_trip < 1.2 * dayz_areaAffect) OR {(_trip > dayz_canDelete)}) then { // too close from current pos (Zed would stay still) or really too far (Zed could be deleted)
	_trip = dayz_areaAffect + random 50;
	_deg = -20 + random 40;
	_posList = (getPosATL _unit) nearObjects ["ReammoBox", 50]; // send Zed to check a lootpile. Get busy...
